...
async def _render_page(cb_or_msg, state: FSMContext, book_id: str, page: int, total: int):
    ...
    await repo.set_page(user_id, book_id, page)
...
async def open_from_list(cb: types.CallbackQuery, state: FSMContext):
    ...
    repo = get_books_repository()
    stored_page = await repo.get_page(cb.from_user.id, book_id)
    page = max(0, min(stored_page, total - 1))
...
async def on_tts(cb: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        s = get_settings()
        tts_pages = int(getattr(s, "tts_pages", 10))
        tts_pages = max(1, min(tts_pages, 20))

        start = page
        end = min(total_pages, page + tts_pages)
